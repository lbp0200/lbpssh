...
void main() {
  ...
  setUp(() {
    ...
    container = ProviderContainer(
      overrides: [
        importExportServiceProvider.overrideWithValue(mockService),
      ],
    );
  });
  ...
  group('ImportExportNotifier', () {
    group('initial state', () {
      test('Given new provider, When created, Then has idle status with no error',
          () {
        // Act (When)
    ...
    group('exportToLocalFile', () {
      test('Given successful export, When called, Then returns file and sets success status',
          () async {
        // Arrange (Given)
        ...
        // Act (When)
        final result =
            await container.read(importExportProvider.notifier).exportToLocalFile();
      ...
      test('Given service throws, When called, Then rethrows and sets error',
          () async {
        // Arrange (Given)
        when(() => mockService.exportToLocalFile())
        ...
        await expectLater(
          () =>
              container.read(importExportProvider.notifier).exportToLocalFile(),
          throwsException,
        );
      });
    });
    ...
    group('importFromLocalFile', () {
      test('Given successful import, When called, Then returns connections and sets success',
          () async {
        // Arrange (Given)
        final connections = [
          SshConnection(
        ...
        // Act (When)
        final result =
            await container.read(importExportProvider.notifier).importFromLocalFile();
    ...
    group('importAndSaveConnections', () {
      test('Given connections, When called, Then delegates to service',
          () async {
        // Arrange (Given)
        final connections = [
          SshConnection(
        ...
        when(() => mockService.importAndSaveConnections(
              any(),
      ...
      test('Given custom options, When called, Then passes options to service',
          () async {
        // Arrange (Given)
        final connections = [
          SshConnection(
        ...
        await container
            .read(importExportProvider.notifier)
            .importAndSaveConnections(connections, overwrite: true, addPrefix: false);
    ...
    group('getExportStats', () {
      test('When called, Then returns stats from service', () {
        ...
        // Act (When)
        final result =
            container.read(importExportProvider.notifier).getExportStats();
    ...
    group('generateExportSummary', () {
      test('When called, Then returns summary string', () {
        ...
        when(() => mockService.generateExportSummary())
            .thenReturn(summary);

        // Act (When)
        final result =
            container.read(importExportProvider.notifier).generateExportSummary();
    ...
    group('resetStatus', () {
      test('When called, Then resets service and state', () {
        ...
        expect(container.read(importExportProvider).status,
