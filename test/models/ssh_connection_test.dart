...
void main() {
  ...
  group('Connection Validation Logic', () {
    test(
      'Given required fields, When creating connection, Then validates correctly',
      () {
        final connection = SshConnection(
          id: 'valid-test',
  ...
  group('SshConnection Creation', () {
    ...
    test(
      'Given connection, When serializing to JSON, Then produces correct JSON',
      () {
        final connection = SshConnection(
          id: 'test-id',
          name: 'Test Connection',
    ...
    test(
      'Given valid JSON, When deserializing, Then creates connection correctly',
      () {
        final json = {
          'id': 'test-id',
          'name': 'Test Connection',
  ...
  group('JumpHostConfig Creation', () {
    ...
    test(
      'Given all fields, When serializing JumpHostConfig, Then produces JSON',
      () {
        final config = JumpHostConfig(
          host: 'jump.example.com',
          port: 2222,
    ...
    test(
      'Given valid JSON, When deserializing JumpHostConfig, Then creates correctly',
      () {
        final json = {
          'host': 'jump.example.com',
          'port': 2222,
  ...
  group('Socks5ProxyConfig Serialization', () {
    test(
      'Given all fields, When serializing Socks5ProxyConfig, Then deserializes correctly',
      () {
        ...
        final json = config.toJson();
        final deserialized = Socks5ProxyConfig.fromJson(json);

        expect(deserialized.host, 'proxy.example.com');
        expect(deserialized.port, 1080);
        expect(deserialized.username, 'proxyuser');
        expect(deserialized.password, 'proxypass');
      },
    );

    test(
      'Given null optional fields, When serializing Socks5ProxyConfig, Then preserves nulls',
      () {
        final config = Socks5ProxyConfig(host: 'proxy.example.com');
        ...
        expect(deserialized.host, 'proxy.example.com');
        expect(deserialized.port, 1080);
        expect(deserialized.username, null);
        expect(deserialized.password, null);
      },
    );

    test(
      'Given no port specified, When creating Socks5ProxyConfig, Then defaults to 1080',
      () {
        final config = Socks5ProxyConfig(host: 'proxy.example.com');
    ...
    test('Given config, When calling copyWith, Then creates modified copy', () {
      ...
      final copy = original.copyWith(port: 2080, password: 'newpass');

      expect(copy.host, 'proxy.example.com');
      expect(copy.port, 2080);
      expect(copy.username, 'user');
      expect(copy.password, 'newpass');
    });

    test(
      'Given config, When calling copyWith with no args, Then preserves all fields',
      () {
        final original = Socks5ProxyConfig(
          host: 'proxy.example.com',
          port: 2080,
    ...
    test(
      ...
      () {
        ...
        final jsonStr = jsonEncode(connection.toJson());
        final deserialized = SshConnection.fromJson(
          jsonDecode(jsonStr) as Map<String, dynamic>,
        );
        expect(deserialized.socks5Proxy, isNotNull);
        expect(deserialized.socks5Proxy!.host, 'proxy.example.com');
        expect(deserialized.socks5Proxy!.port, 1080);
      },
    );

    test(
      'Given connection without proxy, When serializing, Then socks5Proxy is null',
      () {
        final connection = SshConnection(
          id: 'no-proxy',
          name: 'No Proxy',
        ...
        expect(deserialized.socks5Proxy, isNull);
      },
    );
  });
  ...
  group('AuthType', () {
    test(
      'Given AuthType values, When converting to string, Then produces correct values',
      () {
